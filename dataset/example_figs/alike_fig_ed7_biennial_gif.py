...
#to convert in GIF once all PNGs are generated:
# "convert -delay 150 -loop 0 ED_Figure_7_2*.png Fig_ED7_biennial_gif.gif"
...
#EXAMPLE YEARLY
periods = ['20'+str(2*i).zfill(2)+'-01-01_20'+str(2*i+2).zfill(2)+'-01-01' for i in range(10)]

out_dir = '/data/icesat/travail_en_cours/romain/results/figures/'
...
def add_inset(fig,extent,pos,bounds,label=None,polygon=None,anom=None,draw_cmap_y=None,hillshade=True,markup_sub=None,sub_pos=None,sub_adj=None):
    ...
    if hillshade:
        ...
        sub_ax.imshow(hs_tmp[:, :], extent=ext, transform=ccrs.Robinson(), cmap=cmap2, zorder=2,
                      interpolation='nearest')
        sub_ax.imshow(hs_tmp_nl[:, :], extent=ext, transform=ccrs.Robinson(), cmap=cmap22, zorder=2,
                      interpolation='nearest')
    ...
    if anom is not None:
        ...
        if anom in ['dhs_1','dhs_2','dhs_3','dhs_4']:
            ...
            lab = 'Mean elevation change rate (m yr$^{-1}$)'
    ...
    if markup_sub is not None and anom in ['dhs_1','dhs_2','dhs_3','dhs_4']:
        ...
        sub_ax.text(rob_x,rob_y,markup_sub,
                 ...
                 transform=ccrs.Robinson(), color='black',fontsize=6,bbox=dict(facecolor='white', alpha=1,linewidth=0.35,pad=1.5),fontweight='bold',zorder=25)
...
def add_compartment_world_map(fig,pos,anom=None,ymarg=None,label=None,label2=None):
    ...
    out_axes_2 = fig.add_axes([pos[0],pos[1]+pos[3]-0.035,pos[2],0])
    ...
    if label2 is not None:
        out_axes_2.text(0.5, 0, label2, transform=out_axes_2.transAxes, fontsize=8, fontweight='bold', ha='center', va='bottom',
              zorder=30,bbox=dict(facecolor='white', alpha=1,linewidth=0.5,pad=1.5))
...
for period in periods:
    ...
    fig_width_inch = 7.2
    fig = plt.figure(figsize=(fig_width_inch, 1.15 * fig_width_inch / 1.9716))

    list_anom = ['dhs_1']
    labs=['']
    labs2 = [period[0:4]+'-'+str(int(period.split('_')[-1][0:4])-1)]
    out_png = os.path.join(out_dir,'ED_Figure_7_'+period+'.png')


    df_tmp = df[df.period==period]
    ...
    dhs_1 = df_tmp.dhdt.tolist()
    errs_1 = df_tmp.err_dhdt.tolist()
    ...
    axleg5 = fig.add_axes([-0.32, -0.4225, 1, 1], projection=ccrs.Robinson(), label='legend_5')
    ...
    f1 = np.sqrt(((1 / min(max(0.5, 0.25), 1) ** 2 - 1 / 1 ** 2) / (1 / 0.25 ** 2 - 1 / 1 ** 2))) * (
                1 - np.sqrt(0.1)) + np.sqrt(0.1)
    axleg5.add_patch(
        mpatches.Rectangle((-f1 * 500000 / 2 + 2500000, -f1 * 500000 / 2), f1 * 500000, f1 * 500000, edgecolor='black',
                           linewidth=0.35,
                           transform=ccrs.Robinson(), facecolor='None', alpha=1))
    ...
    axleg5.add_patch(mpatches.Rectangle((-np.sqrt(0.1) * 500000 / 2, -np.sqrt(0.1) * 500000 / 2), np.sqrt(0.1) * 500000,
                                        np.sqrt(0.1) * 500000, edgecolor='black',
                                        linewidth=0.35,
                                        transform=ccrs.Robinson(), facecolor='None', alpha=1))
    ...
    sm = plt.cm.ScalarMappable(cmap=cmap_cus, norm=norm)
    sm.set_array([])
    cb = plt.colorbar(sm, cax=cbaxes, ticks=[-1.4, -0.8, -0.4, 0, 0.2, 0.4, 0.6], orientation='horizontal',
    ...
    print('Figure for period '+period+ ' saved.')
