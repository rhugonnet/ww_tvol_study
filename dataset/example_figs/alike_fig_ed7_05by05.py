...
in_csv = '/data/icesat/travail_en_cours/romain/results/vol_final/dh_world_tiles_0.5deg.csv'
...
latmins = df_tmp.tile_latmin.values
lonmins = df_tmp.tile_lonmin.values
areas = df_tmp.area.tolist()
# areas = [area/1000000 for  area in areas]

dhs_1 = df[df.period=='2000-01-01_2005-01-01'].dhdt.tolist()
dhs_2 = df[df.period=='2005-01-01_2010-01-01'].dhdt.tolist()
dhs_3 = df[df.period=='2010-01-01_2015-01-01'].dhdt.tolist()
dhs_4 = df[df.period=='2015-01-01_2020-01-01'].dhdt.tolist()
...
def add_inset(fig,extent,pos,bounds,label=None,polygon=None,anom=None,draw_cmap_y=None,hillshade=True,markup_sub=None,sub_pos=None,sub_adj=None):
    ...
    if anom is not None:
        ...
        for i in range(len(latmins)):
            ...
            f=np.sqrt(((1/min(max(errs[i],0.25),1)**2-1/1**2)/(1/0.25**2-1/1**2)))*(1-np.sqrt(0.3))
            ...
            xl = np.sqrt(0.3)*s[0] + f*s[0]
            yl = np.sqrt(0.3)*s[1] + f*s[1]
...
f1 = np.sqrt(((1/min(max(0.5,0.25),1)**2-1/1**2)/(1/0.25**2-1/1**2)))*(1-np.sqrt(0.3)) + np.sqrt(0.3)
...
axleg5.text(2500000,-1200000,'56%',horizontalalignment='center',verticalalignment='center')

axleg5.add_patch(mpatches.Rectangle((-np.sqrt(0.3)*500000/2,-np.sqrt(0.3)*500000/2), np.sqrt(0.3)*500000, np.sqrt(0.3)*500000, edgecolor='black',
                                linewidth=0.35,
                                transform=ccrs.Robinson(), facecolor='None', alpha=1))

axleg5.text(0,-600000,'> 2 m yr$^{-1}$',horizontalalignment='center',verticalalignment='center')
axleg5.text(0,-1200000,'30%',horizontalalignment='center',verticalalignment='center')
...
sm = plt.cm.ScalarMappable(cmap=cmap_cus, norm=norm)
sm.set_array([])
cb = plt.colorbar(sm, cax=cbaxes, ticks=[-1.4, -0.8, -0.4, 0, 0.2, 0.4, 0.6], orientation='horizontal', extend='both', shrink=0.9)
...
plt.savefig('/data/icesat/travail_en_cours/romain/results/figures/ED_Figure_7_alike_05by05.png',dpi=400)
