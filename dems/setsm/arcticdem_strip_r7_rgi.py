...
from demstripproducts import ArcticDEM_strip_r7
...
final_dir = '/data/icesat/travail_en_cours/romain/data/dems/arcticdem'
arcdem_directory_listing = '/data/icesat/travail_en_cours/romain/ww_tvol_study/worldwide/global/SETSM_directory_listing/ArcticDEM_r7_directory_listing.csv'
...
#list of ArcticDEM tiles: directory listing
...
print('Found '+str(len(list_common_tiles))+' ArcticDEM r7 tiles intersecting glaciers.')
...
print(list_remaining_tiles)

print('Found '+str(len(list_remaining_tiles))+' ArcticDEM r7 tiles not yet processed.')
...
def get_process_tile(tmp_dir,tile):
    ...
    while check and u<5:
        ...
        if u>1:
            time.sleep(600)
        #download strip data
        cmd = 'wget -r -N -nH -np -R index.html* --cut-dirs=6 http://data.pgc.umn.edu/elev/dem/setsm/ArcticDEM/geocell/v3.0/2m/'+arc_tile_name+'/ -P '+tmp_dir
    ...
    #otherwise, process
        ...
        with open(mosaic_log_file,'w') as mos:
            with stdout_redirected(to=mos), merged_stderr_stdout():
                try:
                    ArcticDEM_strip_r7(os.path.join(tmp_dir,'2m'),arc_tile_name,tmp_dir_out,mosaic_coreg_segm=True,tgt_EPSG=None,tgt_res=[30,-30],nodata_out=-9999,interp_method='bilinear',geoid=False,rm_tar=True,downsample=True)
                except Exception:
