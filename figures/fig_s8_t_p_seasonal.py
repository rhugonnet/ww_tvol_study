...
in_csv='/data/icesat/travail_en_cours/romain/results/era5/final/mb_climate_1deg_change_20202010_20102000_all_700hpa_temp_seasonal.csv'
# in_csv_2 = '/data/icesat/travail_en_cours/romain/results/era5/final/mb_climate_1deg_change_20202010_20102000_all_700hpa_temp.csv'
fig_width_inch = 8
fig = plt.figure(figsize=(fig_width_inch, 2.55 * fig_width_inch / 1.9716))
...
out_png = os.path.join(os.path.dirname(in_csv),'fig_s8_'+os.path.splitext(os.path.basename(in_csv))[0]+'.png')
...
dps = list(df.d_P.values) #- df_2.d_P.values)
dts = list(df.d_T.values) #- df_2.d_T.values)
...
def add_inset(fig,extent,pos,bounds,label=None,polygon=None,anom=None,draw_cmap_y=None,hillshade=True,markup_sub=None,sub_pos=None,sub_adj=None):
    ...
    if anom is not None:
        ...
        if anom == 'dh':
            ...
            lab = 'Mean elevation change (m yr$^{-1}$)'
            ...
            vals = dts
            lab = 'Decadal difference in summer temperature (K)'
            ...
            vals = dps
            lab = 'Decadal difference in winter precipitation (m)'
        ...
        if draw_cmap_y is not None:
            ...
            cbaxes = sub_ax_2.inset_axes([0,0.9,1,0.35],label='legend_'+label)
...
def add_compartment_world_map(fig,pos,anom=None,ymarg=None,label=None):
    ...
    add_inset(fig, [-179.99,179.99,-89.99,89.99], [-0.765, yb-0.4694*yr, 2.7, 2.7*yr],bounds=bounds_sa,label='South America',anom=anom,markup_sub='16-17',sub_pos='lb',sub_adj=(20000,20000))
    ...
    out_axes = fig.add_axes([pos[0], pos[1]-0.005 + pos[3], 0.2, ymarg])
    out_axes.set_xticks([])
    out_axes.set_yticks([])
