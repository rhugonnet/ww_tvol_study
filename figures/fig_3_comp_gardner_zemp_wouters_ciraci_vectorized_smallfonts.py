...
for i in range(len(r_list)):
    ...
    if ~np.isnan(df_ext.wout[i]):
        ax.errorbar(shift_x+x_axis[nb*i+1], df_w.dmdtda.values[0],2*df_w.err_dmdtda.values[0],fmt='o',color=plt.cm.Greys(0.8),capsize=1.2,zorder=30,ms=2.5,elinewidth=0.6,mew=0.6)
    ...
    #     ax.plot([shift_x + x_axis[4 * i + 3] - 0.5, shift_x+x_axis[4 * i + 3] + 0.5], [dmdtda] * 2, color=col,linewidth=0.5)
...
ax.vlines(np.arange(-1,17*5,5),-3,3,colors='grey',alpha=0.7,linewidth=0.35,linestyles='dashed')


ax.hlines(0,-5,17*5+5,linestyles='dashed',colors=plt.cm.Greys(0.9),linewidth=0.5)
...
ax.set_ylim([-1.5,0.45])
ax.text(0.025, 0.95, 'a', transform=ax.transAxes, ha='left', va='top',fontweight='bold',fontsize=8)

p3 = ax.plot([], [], color='tab:red', linewidth=0.5)
p4 = ax.fill([], [], color='tab:red', alpha=0.45,linewidth=0.25)
p5 = ax.plot([], [], color='tab:cyan', linewidth=0.5)
p6 = ax.fill([], [], color='tab:cyan', alpha=0.45,linewidth=0.25)
p1 = ax.plot([], [], color='tab:orange', linewidth=0.5)
p2 = ax.fill([], [], color='tab:orange', alpha=0.45,linewidth=0.25)
p7 = ax.plot([], [], color='tab:purple', linewidth=0.5)
p8 = ax.fill([], [], color='tab:purple', alpha=0.45,linewidth=0.25)
...
for i in range(len(tlims)-1):
    ...
    ax1.fill_between([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]+2*df_p.err_dmdtda.values[0]]*2,[df_p.dmdtda.values[0]-2*df_p.err_dmdtda.values[0]]*2,color=plt.cm.Greys(0.9),alpha=0.3,linewidth=0.25)
    ax1.plot([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]]*2,color=plt.cm.Greys(0.9),lw=0.35)
    ...
    # ax1.plot([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]]*2,color=plt.cm.Purples(0.9),linewidth=0.5,linestyle='dotted')
...
for i in range(len(tlims)-1):
    df_p = df_glob[df_glob.period==str(tlims[i])+'_'+str(tlims[i+1])]
    ax1.fill_between([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]+2*df_p.err_dmdtda.values[0]]*2,[df_p.dmdtda.values[0]-2*df_p.err_dmdtda.values[0]]*2,color=plt.cm.Greys(0.9),alpha=0.5,linewidth=0.25)
    ax1.plot([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]]*2,color=plt.cm.Greys(0.9),linewidth=0.75)


ax1.fill_between(tlim_zemp,[dmdtda_zemp+err_zemp]*2,[dmdtda_zemp-err_zemp]*2,color='tab:orange',alpha=0.45,linewidth=0.25)
ax1.plot(tlim_zemp, [dmdtda_zemp] * 2, color='tab:orange',lw=0.75)
...
ax1.fill_between(tlim_gardner,[dmdtda_gard+err_gard]*2,[dmdtda_gard-err_gard]*2,color='tab:red',alpha=0.45,linewidth=0.25)
ax1.plot(tlim_gardner, [dmdtda_gard] * 2, color='tab:red',lw=0.75)
...
ax1.plot([np.datetime64('2000-01-01'),np.datetime64('2020-01-01')],[0,0],linestyle='dashed',color=plt.cm.Greys(0.9),linewidth=0.5)
...
ax1.set_yticks([])
ax.set_ylabel('Specific mass change rate (m w.e yr$^{-1}$)',labelpad=0.25)
ax1.set_xlabel('Year\n(Global total)')
ax1.tick_params(width=0.35,length=2.5)


ax1 = fig.add_subplot(grid[:-3, 11:14])
...
for i in range(len(tlims)-1):
    df_p = df_per[df_per.period==str(tlims[i])+'_'+str(tlims[i+1])]
    ax1.fill_between([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]+2*df_p.err_dmdtda.values[0]]*2,[df_p.dmdtda.values[0]-2*df_p.err_dmdtda.values[0]]*2,color=plt.cm.Greys(0.9),alpha=0.5,linewidth=0.25)
    ax1.plot([tlims[i],tlims[i+1]],[df_p.dmdtda.values[0]]*2,color=plt.cm.Greys(0.9),linewidth=1)


ax1.fill_between(tlim_cira,[dmdtda_cira+err_cira]*2,[dmdtda_cira-err_cira]*2,color='tab:purple',alpha=0.45,zorder=3,linewidth=0.25)
ax1.plot(tlim_cira, [dmdtda_cira] * 2, color='tab:purple',zorder=3,lw=0.75)

ax1.fill_between(tlim_wouters,[dmdtda_wouters+err_wouters]*2,[dmdtda_wouters-err_wouters]*2,color='tab:cyan',alpha=0.45,zorder=2,linewidth=0.25)
ax1.plot(tlim_wouters, [dmdtda_wouters] * 2, color='tab:cyan',zorder=2,lw=0.75)
# ax1.errorbar(tlim_zemp[0]+(tlim_zemp[1]-tlim_zemp[0])/2,df_z_glo.dmdtda.values[0],2*df_z_glo.err_dmdtda.values[0],fmt='o',color=plt.cm.Greys(0.8),capsize=3,zorder=30)
ax1.fill_between(tlim_gardner,[dmdtda_gard+err_gard]*2,[dmdtda_gard-err_gard]*2,color='tab:red',alpha=0.45,zorder=4,linewidth=0.25)
ax1.plot(tlim_gardner, [dmdtda_gard] * 2, color='tab:red',zorder=4,lw=0.75)
# ax1.errorbar(tlim_gardner[0]+(tlim_gardner[1]-tlim_gardner[0])/2,df_g_glo.dmdtda.values[0],2*df_g_glo.err_dmdtda.values[0],fmt='o',color=plt.cm.Greys(0.8),capsize=3,zorder=30)
ax1.fill_between(tlim_zemp,[dmdtda_zemp+err_zemp]*2,[dmdtda_zemp-err_zemp]*2,color='tab:orange',alpha=0.45,zorder=5,linewidth=0.25)
ax1.plot(tlim_zemp, [dmdtda_zemp] * 2, color='tab:orange',zorder=5,lw=0.75)
...
ax1.set_ylabel('Specific mass change rate (m w.e yr$^{-1}$)',labelpad=0.25)
ax1.text(0.075, 0.95, 'b', transform=ax1.transAxes, ha='left', va='top',fontweight='bold',fontsize=8)
ax1.tick_params(width=0.35,length=2.5)

p10 = ax.plot([], [], color=plt.cm.Greys(0.9), linewidth=0.35)
p11 = ax.fill([], [], color=plt.cm.Greys(0.9), alpha=0.3,linewidth=0.25)

p12= ax.plot([], [], color=plt.cm.Greys(0.9), linewidth=1)
p13 = ax.fill([], [], color=plt.cm.Greys(0.9), alpha=0.5,linewidth=0.25)
...
l = ax1.legend([(p3[0],p4[0]),(p5[0],p6[0]),(p7[0], p8[0]),(p1[0], p2[0]),(p10[0],p11[0]),(p12[0],p13[0])], ['Gardner et al.'+'$^{5}$','Wouters et al.'+'$^{19}$','Ciracì et al.'+'$^{20}$','Zemp et al.'+'$^{21}$','This study (yearly)'+'$^{\,}$','This study (5-year)'+'$^{\,}$']
