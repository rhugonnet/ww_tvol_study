...
def line_main_to_inset(main_pos,inset_pos,inset_verts):
    ...
    main_mod_y = (unit_vert_y*main_pos[3] + main_pos[1])/inset_pos[3]
      ...
      #
    # shade_ax.plot(main_mod_x*ext_robin_x - limits_robin[1][0],main_mod_y*ext_robin_y - limits_robin[3][1],color=col_contour,linestyle='dashed',linewidth=1.5,dashes=(3,1.5))
...
def color_ribbon(col,position,rw,rl,label,ticks=None):
    ...
    for i in range(nb_rect):
        sub_ax_rect.add_patch(
            mpatches.Rectangle((vect_rob_x[i], rob_y), rw, rl, linewidth=1, edgecolor='None', facecolor=col[i],
                               transform=ccrs.Robinson(),zorder=30))

    sub_ax_rect.add_patch(
            mpatches.Rectangle((vect_rob_x[0], rob_y), nb_rect*rw, rl, linewidth=1, edgecolor='black', facecolor='None',
                               transform=ccrs.Robinson(),zorder=30))
    ...
    if ticks is not None:
        ...
        for i in np.arange(0, nb_rect + 1, 4):
            sub_ax_rect.add_patch(mpatches.Arrow(vect_rob_x[i], y_rect, 0, rl * tick_rat, color='black', linewidth=1,
                                                 transform=ccrs.Robinson(), zorder=30))
...
def world_ribbon(col,tdh,position,rw,rl):
    ...
    for i in range(nb_rect):
        ...
        sub_ax_rect.add_patch(
            mpatches.Rectangle((vect_rob_x[i], rob_y-vect), rw, vect, linewidth=1, edgecolor='black', facecolor=col[i],
                               transform=ccrs.Robinson(), zorder=30))
    ...
    sub_ax_rect.add_patch(
        mpatches.Arrow(vect_rob_x[0], rob_y, 0, -5/4*rl*np.max(np.abs(tdh)),10000, color='black', linewidth=1,
                       transform=ccrs.Robinson(), zorder=30))

    sub_ax_latlon = fig.add_axes([0, 0, 1, 1],
                               projection=ccrs.PlateCarree(), label='world2')
    sub_ax_latlon.text(-143.75,-72.5,'0',ha='right',va='center',transform=ccrs.PlateCarree(),color='black',fontsize=12)
    sub_ax_latlon.text(-143.75,-77.5,'-0.25',ha='right',va='center',transform=ccrs.PlateCarree(),color='black',fontsize=12)
    sub_ax_latlon.text(-143.75,-82.5,'-0.50',ha='right',va='center',transform=ccrs.PlateCarree(),color='black',fontsize=12)
    sub_ax_latlon.text(-142.12,-89,'m yr$^{-1}$',ha='center',va='center',transform=ccrs.PlateCarree(),color='black',fontsize=12)
    sub_ax_latlon.annotate('', xy=(-142.12, -87), xytext=(-142.12, -84),
                         xycoords=ccrs.PlateCarree()._as_mpl_transform(sub_ax_latlon),
                         arrowprops=dict(facecolor='black', width=0.25,headwidth=5,headlength=5))

    verts = mpath.Path(rect_units_to_verts([-178, -88, 10, 10]))

    sub_ax_latlon.set_boundary(verts, transform=sub_ax_latlon.transAxes)

    # sub_ax_rect.plot([vect_rob_x[0], vect_rob_x[-1]], [rob_y-0.56*rl,rob_y-0.56*rl],transform=ccrs.Robinson(),color='black', linewidth=1, linestyle='dashed',zorder=30)

    # sub_ax_rect.add_patch(
    #     mpatches.Arrow(vect_rob_x[0], rob_y, rw*0.5, 0, color='black', linewidth=1,
    #                    transform=ccrs.Robinson(), zorder=30))
    sub_ax_rect.add_patch(
        mpatches.Arrow(vect_rob_x[0] - rw * 0.5, rob_y, rw * 0.5, 0, color='black', linewidth=1,
                       transform=ccrs.Robinson(), zorder=30))
    sub_ax_rect.add_patch(
        mpatches.Arrow(vect_rob_x[0]-rw*0.5, rob_y-rl*0.25, rw*0.5, 0, color='black', linewidth=1,
                       transform=ccrs.Robinson(), zorder=30))
    sub_ax_rect.add_patch(
        mpatches.Arrow(vect_rob_x[0]-rw*0.5, rob_y-rl*0.5, rw*0.5, 0, color='black', linewidth=1,
                       transform=ccrs.Robinson(), zorder=30))
    # sub_ax_rect.add_patch(
    #     mpatches.Arrow(vect_rob_x[0], rob_y-rl*0.75, rw*0.5, 0, color='black', linewidth=1,
    #                    transform=ccrs.Robinson(), zorder=30))

    sub_ax_rect.plot([vect_rob_x[0], vect_rob_x[0]], [rob_y, rob_y+2150000],transform=ccrs.Robinson(),color='black', linewidth=1, linestyle='dashed',zorder=30)
    sub_ax_rect.plot((vect_rob_x[-1], vect_rob_x[-1]),(rob_y,rob_y+2150000),transform=ccrs.Robinson(), color='black', linewidth=1, linestyle='dashed', zorder=30)

    sub_ax_rect.text(vect_rob_x[0] + 100000, rob_y + 1300000, '-0.36',
                     horizontalalignment='left', verticalalignment='bottom',
                     transform=ccrs.Robinson(), color='black', fontsize=14,
                     fontweight='bold')
    ...
    sub_ax_rect.text(vect_rob_x[0], rob_y + 2200000, '2000',
                     horizontalalignment='center', verticalalignment='bottom',
                     transform=ccrs.Robinson(), color='black', fontsize=14,
                     fontweight='bold',bbox= dict(boxstyle='round',facecolor='white', alpha=1),zorder=30)
...
def add_reg_label(rect_center,text,label):
    ...
    #     mpatches.Rectangle((rect_center[0]-width/2, rect_center[1]-height), width, height, linewidth=1, edgecolor='grey', facecolor='white',
...
def add_color_ribbon_circ(position,tdh,dmdt,valid_obs_py,label,perc_dm_tw,perc_area_tw,text,offset=(0,0),width_ribbon=100000,height_ribbon=300000):
    ...
    if perc_area_tw>0:
        sub_ax_cerc.add_patch(
            mpatches.Wedge(center=xy, r=rad, theta1=theta1_dm, theta2=theta2_dm ,facecolor=plt.cm.Blues(0.3), alpha=1, transform=ccrs.Robinson(), zorder=30,edgecolor=plt.cm.Greys(0.4),linewidth=1))
        sub_ax_cerc.add_patch(
            mpatches.Wedge(center=xy, r=rad, theta1=theta2_dm, theta2=theta1_dm, facecolor='whitesmoke', alpha=1,
                           transform=ccrs.Robinson(), zorder=30, edgecolor=plt.cm.Greys(0.4), linewidth=1))
        sub_ax_cerc.add_patch(
        ...
        sub_ax_cerc.add_patch(
            mpatches.Circle(xy=xy, radius=rad+width_area, facecolor='None', alpha=1,
                           transform=ccrs.Robinson(), zorder=30, edgecolor=plt.cm.Greys(0.8), linewidth=1))
        sub_ax_cerc.add_patch(
...
df = pd.read_csv('/home/atom/ongoing/work_worldwide/vol/tile/subreg_multann.csv')
...
for a in [-0.5, -5, -50]:
    ...
    if a != -50:
        axleg.add_patch(mpatches.Circle(xy=[-700000+rad_tot+rad+u*800000,0],radius=rad,edgecolor='k',label=str(a)+' km$^2$', transform = ccrs.Robinson(),fill=False, zorder=30))
        axleg.text(-700000 + rad_tot + rad + u * 800000, -rad - 100000, str(a), transform=ccrs.Robinson(),
                   horizontalalignment='center',
                   verticalalignment='top', fontsize=12)
    else:
        rad_tot += 250000
        axleg.add_patch(mpatches.Wedge(center=[-700000+rad_tot+rad+u*800000,0],r=rad,theta1=90,theta2=270,facecolor=plt.cm.Blues(0.3),edgecolor=plt.cm.Greys(0.4),label=str(a)+' km$^2$', transform = ccrs.Robinson(), zorder=30))
        axleg.add_patch(mpatches.Wedge(center=[-700000+rad_tot+rad+u*800000,0],r=rad,theta1=270,theta2=90,facecolor='whitesmoke',edgecolor=plt.cm.Greys(0.4),label=str(a)+' km$^2$', transform = ccrs.Robinson(), zorder=30))
        axleg.add_patch(mpatches.Wedge(center=[-700000+rad_tot+rad+u*800000,0],r=rad+100000,theta1=90,theta2=270,width=100000,facecolor=plt.cm.Blues(0.9),edgecolor=plt.cm.Greys(0.4),label=str(a)+' km$^2$', transform = ccrs.Robinson(), zorder=30))
        axleg.add_patch(mpatches.Wedge(center=[-700000+rad_tot+rad+u*800000,0],r=rad+100000,theta1=270,theta2=90,width=100000,facecolor='silver',edgecolor=plt.cm.Greys(0.4),label=str(a)+' km$^2$', transform = ccrs.Robinson(), zorder=30))
        axleg.add_patch(mpatches.Circle(xy=[-700000+rad_tot+rad+u*800000,0],radius=rad,edgecolor=plt.cm.Greys(0.8),label=str(a)+' km$^2$', transform = ccrs.Robinson(),fill=False, zorder=30))
        axleg.add_patch(mpatches.Circle(xy=[-700000+rad_tot+rad+u*800000,0],radius=rad+100000,edgecolor=plt.cm.Greys(0.8),label=str(a)+' km$^2$', transform = ccrs.Robinson(),fill=False, zorder=30))
...
axleg.text(5, -14, 'Mass change rate\nover 2000-2019 (Gt yr$^{-1}$)', transform=ccrs.Geodetic(),horizontalalignment='center',verticalalignment='center',fontsize=12)
axleg.text(60,-13.5,'Surface ratio',transform=ccrs.Geodetic(),horizontalalignment='center',verticalalignment='center',fontsize=12)
axleg.add_patch(mpatches.Arrow(4600000, -1400000,-1600000,0,transform=ccrs.Robinson(),edgecolor='black',linewidth=1,zorder=30))
axleg.text(60,-7,'Mass change\nratio',transform=ccrs.Geodetic(),horizontalalignment='center',verticalalignment='center',fontsize=12)
axleg.add_patch(mpatches.Arrow(4600000,-700000,-1600000,0,transform=ccrs.Robinson(),edgecolor='black',linewidth=1,zorder=30))
axleg.text(60,10,'Land-\nterminating',color='dimgrey',transform=ccrs.Geodetic(),horizontalalignment='center',verticalalignment='center',fontsize=12)
axleg.add_patch(mpatches.Arrow(4600000,1000000,-500000,-100000,transform=ccrs.Robinson(),edgecolor='black',linewidth=1,zorder=30))
axleg.add_patch(mpatches.Arrow(4600000,1000000,-1000000,-500000,transform=ccrs.Robinson(),edgecolor='black',linewidth=1,zorder=30))
axleg.text(60,2.5,'Marine-\nterminating',color=plt.cm.Blues(0.9),transform=ccrs.Geodetic(),horizontalalignment='center',verticalalignment='center',fontsize=12)
axleg.add_patch(mpatches.Arrow(4600000,250000,-2400000,-200000,transform=ccrs.Robinson(),edgecolor='black',linewidth=1,zorder=30))
axleg.add_patch(mpatches.Arrow(4600000,250000,-2850000,-700000,transform=ccrs.Robinson(),edgecolor='black',linewidth=1,zorder=30))
bounds = [-10,10,-10,10]
polygon = poly_from_extent(bounds)
verts = mpath.Path(latlon_extent_to_robinson_axes_verts(polygon))
...
for i in range(nb_rect):
    axleg3.add_patch(
        mpatches.Rectangle((vect_rob_x[i], rob_y), rw, rl/2, linewidth=1, edgecolor='None', facecolor=col[i],
                           transform=ccrs.Robinson(), zorder=30))

axleg3.add_patch(
    mpatches.Rectangle((vect_rob_x[0], rob_y), nb_rect * rw, rl/2, linewidth=1, edgecolor='black', facecolor='None',
                       transform=ccrs.Robinson(), zorder=30))
...
for i in np.arange(0, nb_rect + 1, 4):
    axleg3.add_patch(mpatches.Arrow(vect_rob_x[i], rob_y, 0, rl * tick_rat, color='black', linewidth=1,
                                         transform=ccrs.Robinson(), zorder=30))
    if i % 8 ==0:
        y_low = 100000
    else:
...
axleg3.text(position[0],rob_y-800000,'Annual time series',horizontalalignment='center',verticalalignment='top',fontsize=12)
axleg3.text(position[0],position[1]+200000,'Percentage of area observed\nat least once a year',horizontalalignment='center',verticalalignment='bottom',fontsize=12)
axleg3.text(vect_rob_x[0],position[1]+50000,'0%',horizontalalignment='center',verticalalignment='bottom',fontsize=12)
axleg3.text(vect_rob_x[-1],position[1]+50000,'100%',horizontalalignment='center',verticalalignment='bottom',fontsize=12)
...
axleg5.text(0,-1000000,'Glacier contours\n(shaded: regions)',horizontalalignment='center',verticalalignment='center',fontsize=12)
