...
out_png = '/home/atom/ongoing/work_worldwide/figures/Figure_2_4per_v3.png'
...
# out_png = '/data/icesat/travail_en_cours/romain/results/figures/Figure_1'+period+'.png'
...
def latlon_to_spec_tile_naming(lat,lon):
    ...
    if np.abs(lat)>=74:
        lon_sw=np.floor((lon-0.5)/4)*4
        lat_sw = np.floor((lat-0.5)/4)*4
    elif np.abs(lat) >=60:
        lon_sw = np.floor((lon-0.5)/ 4) * 4
...
list_periods = ['20'+str(i).zfill(2)+'-01-01_20'+str(i+1).zfill(2)+'-01-01' for i in range(20)]

#
# period_1 = '2000-01-01_2005-01-01'
# period_2 = '2005-01-01_2010-01-01'
# period_3 = '2010-01-01_2015-01-01'
# period_4 = '2015-01-01_2020-01-01'

df_all = pd.read_csv(in_csv)
#
# # ind_1 = np.logical_and.reduce((df_all.category=='all',df_all.period==period_1))
# # ind_2 = np.logical_and.reduce((df_all.category=='all',df_all.period==period_2))
# # ind_3 = np.logical_and.reduce((df_all.category=='all',df_all.period==period_3))
# # ind_4 = np.logical_and.reduce((df_all.category=='all',df_all.period==period_4))
#
# # df_1= df_all[ind_1]
# # df_2= df_all[ind_2]
# # df_3= df_all[ind
...
# # df_4= df_all[ind_4]
# #
# # df_1.loc[df_1.perc_area_meas<0.4,'dhdt']=np.nan
# # df_2.loc[df_2.perc_area_meas<0.4,'dhdt']=np.nan
# # df_3.loc[df_3.perc_area_meas<0.4,'dhdt']=np.nan
# # df_4.loc[df_4.perc_area_meas<0.4,'dhdt']=np.nan
#
list_df=[]
for period in list_periods:
    ind = np.logical_and.reduce((df_all.category=='all',df_all.period==period))
    df = df_all[ind]
    df.loc[df.perc_area_meas<0.4,'dhdt']=np.nan
    list_df.append(df)
#
# # tiles = df_all.tile.tolist()
tiles = [latlon_to_SRTMGL1_naming(list_df[0].tile_latmin.values[i],list_df[0].tile_lonmin.values[i]) for i in range(len(list_df[0]))]
areas = list_df[0].area.tolist()
#
# list_dhs = [df.dhdt.tolist() for df in list_df]
# # dhs_1 = df_1.dhdt.tolist()
# # dhs_2 = df_2.dhdt.tolist()
# # dhs_3 = df_3.dhdt.tolist()
# # dhs_4 = df_4.dhdt.tolist()
# #
# # errs_1 = df_1.err_dhdt.tolist()
# # errs_2 = df_2.err_dhdt.tolist()
# # errs_3 = df_3.err_dhdt.tolist()
# # errs_4 = df_4.err_dhdt.tolist()
dhs = list_df[0].dhdt.tolist()
list_final_dh = []
if group_by_spec:
    # for dhs in list_dhs:
    ...
    list_tile_grouped = []
    final_areas = []
    # final_dhs_1, final_dhs_2, final_dhs_3, final_dhs_4 = ([] for i in range(4))
    # final_errs_1, final_errs_2, final_errs_3, final_errs_4 = ([] for i in range(4))
    final_dhs = []
    final_errs=[]
    ...
    for tile in final_tiles:
        ...
        #
        # group_dhs_1 = np.array([dh for dh in dhs_1 if tile in list_tile_grouped[dhs_1.index(dh)]])
        # group_dhs_2 = np.array([dh for dh in dhs_2 if tile in list_tile_grouped[dhs_2.index(dh)]])
        # group_dhs_3 = np.array([dh for dh in dhs_3 if tile in list_tile_grouped[dhs_3.index(dh)]])
        # group_dhs_4 = np.array([dh for dh in dhs_4 if tile in list_tile_grouped[dhs_4.index(dh)]])
        #
        # group_errs_1 = np.array([err for err in errs_1 if tile in list_tile_grouped[errs_1.index(err)]])
        # group_errs_2 = np.array([err for err in errs_2 if tile in list_tile_grouped[errs_2.index(err)]])
        # group_errs_3 = np.array([err for err in errs_3 if tile in list_tile_grouped[errs_3.index(err)]])
        # group_errs_4 = np.array([err for err in errs_4 if tile in list_tile_grouped[errs_4.index(err)]])
        ...
        if np.count_nonzero(~np.isnan(group_dhs))!=0:
            ...
            # final_dhs_1.append(np.nansum(group_dhs_1*group_areas)/np.nansum(group_areas))
            # final_dhs_2.append(np.nansum(group_dhs_2*group_areas)/np.nansum(group_areas))
            # final_dhs_3.append(np.nansum(group_dhs_3*group_areas)/np.nansum(group_areas))
            # final_dhs_4.append(np.nansum(group_dhs_4*group_areas)/np.nansum(group_areas))

        else:
            final_dhs.append(np.nan)

            # final_dhs_1.append(np.nan)
            # final_dhs_2.append(np.nan)
            # final_dhs_3.append(np.nan)
            # final_dhs_4.append(np.nan)

        # final_errs_1.append(np.nansum(group_errs_1**2*group_areas**2)/np.nansum(group_areas)**2)
        # final_errs_2.append(np.nansum(group_errs_2**2*group_areas**2)/np.nansum(group_areas)**2)
        # final_errs_3.append(np.nansum(group_errs_3**2*group_areas**2)/np.nansum(group_areas)**2)
        # final_errs_4.append(np.nansum(group_errs_4**2*group_areas**2)/np.nansum(group_areas)**2)
    ...
    final_areas = [area/1000000 for _, area in sorted(zip(final_tiles,final_areas))]
    final_dhs = [dh for _, dh in sorted(zip(final_tiles,final_dhs))]

    list_final_dh.append(final_dhs)
#
# df_tmp = pd.DataFrame()
# for final_dh in list_final_dh:
#     df_tmp[list_periods[list_final_dh.index(final_dh)]]=np.array(final_dh)
#
# df_tmp.to_csv('/home/atom/ongoing/work_worldwide/figures/tmp_tiles.csv')

df_tmp = pd.read_csv('/home/atom/ongoing/work_worldwide/figures/tmp_tiles.csv')
list_final_dh = []
for i in range(len(list_periods)):
    list_final_dh.append(df_tmp.iloc[:,i+1])
...
final_tiles = sorted(final_tiles)
...
def add_inset(fig,extent,position,bounds=None,label=None,polygon=None,shades=True, hillshade=True, list_shp=None, main=False, markup=None,markpos='left',markadj=0,markup_sub=None,sub_pos='lt'):
    ...
    if not main:
        for i in range(len(final_tiles)):
            lat, lon = SRTMGL1_naming_to_latlon(final_tiles[i])
            if group_by_spec:
            ...
            rad = 50000 + np.sqrt(final_areas[i]) * fac
            # cmin = -1
            # cmax = 1
            ...
            if ~np.isnan(list_final_dh[0][i]) and final_areas[i]>0.2:# and errs[i]<0.5: #and ((areas[i]>=5.) or label in ['Mexico','Indonesia','Africa']):

                # dhdt_1 = dhs_1[i]
                # dhdt_2 = dhs_2[i]
                # dhdt_3 = dhs_3[i]
                # dhdt_4 = dhs_4[i]
                # dhdt_tot = np.nanmean([dhdt_1,dhdt_2,dhdt_3,dhdt_4])
                #
                # dhdt_col_1 = max(0.0001,min(0.9999,(dhdt_1 - min(col_bounds))/(max(col_bounds)-min(col_bounds))))
                # dhdt_col_2 = max(0.0001,min(0.9999,(dhdt_2 - min(col_bounds))/(max(col_bounds)-min(col_bounds))))
                # dhdt_col_3 = max(0.0001,min(0.9999,(dhdt_3 - min(col_bounds))/(max(col_bounds)-min(col_bounds))))
                # dhdt_col_4 = max(0.0001,min(0.9999,(dhdt_4 - min(col_bounds))/(max(col_bounds)-min(col_bounds))))
                # dhdt_col_tot = max(0.0001,min(0.9999,(dhdt_tot - min(col_bounds))/(max(col_bounds)-min(col_bounds))))
                # ind = max(0, min(int((dhs[i]/20. - cmin) / (cmax - cmin) * 100), 99))
                # if dhs[i]>=0:
                #     ind = max(0, min(int((np.sqrt(dhs[i]/20.) - cmin) / (cmax - cmin) * 100), 99))
                # else:
                #     ind = max(0, min(int((-np.sqrt(-dhs[i]/20.) - cmin) / (cmax - cmin) * 100), 99))
                # col1 = cmap_cus(dhdt_col_1)
                # col2 = cmap_cus(dhdt_col_2)
                # col3 = cmap_cus(dhdt_col_3)
                # col4 = cmap_cus(dhdt_col_4)

                list_col = []
                for dhs in list_final_dh:
                    dhdt = dhs[i]
                    dhdt_col = max(0.0001,
                                     min(0.9999, (dhdt - min(col_bounds)) / (max(col_bounds) - min(col_bounds))))
                    col_tot = cmap_cus(dhdt_col)
                    list_col.append(col_tot)

            elif areas[i] <= 5:
                continue
            else:
                col_tot= plt.cm.Greys(0.7)
                # col = 'black'
            ...
            for i in range(len(list_col)):
                col=list_col[i]
                sub_ax.add_patch(
                    mpatches.Wedge(center=xy, r=rad+w, width=w, theta1=360-(i+1)*(360/len(list_col)), theta2=360-(i)*(360/len(list_col)),
                                   color=col, alpha=1,
                                   transform=ccrs.Robinson(), zorder=30))
                # sub_ax.add_patch(
            #     mpatches.Wedge(center=xy, r=rad+w, theta1=0, theta2=90,
            #                    color=col1, alpha=1,
            #                    transform=ccrs.Robinson(), zorder=30,edgecolor=plt.cm.Greys(0.9),linewidth=0.75))
            # sub_ax.add_patch(
            #     mpatches.Wedge(center=xy, r=rad+w, theta1=270, theta2=360,
            #                    color=col2, alpha=1,
            #                    transform=ccrs.Robinson(), zorder=30,edgecolor=plt.cm.Greys(0.9),linewidth=0.75))
            # sub_ax.add_patch(
            #     mpatches.Wedge(center=xy, r=rad+w, theta1=180, theta2=270,
            #                    color=col3, alpha=1,
            #                    transform=ccrs.Robinson(), zorder=30,edgecolor=plt.cm.Greys(0.9),linewidth=0.75))
            # sub_ax.add_patch(
            #     mpatches.Wedge(center=xy, r=rad+w, theta1=90, theta2=180,
            #                    color=col4, alpha=1,
            #                    transform=ccrs.Robinson(), zorder=30,edgecolor=plt.cm.Greys(0.9),linewidth=0.75))

            # sub_ax.add_patch(
            #     mpatches.Circle(xy=xy, radius=rad, facecolor=col_tot, alpha=1, transform=ccrs.Robinson(), zorder=30,
            #                     edgecolor=plt.cm.Greys(0.9), linewidth=0.75))
...
# sub_ax.add_patch(
    ...
    if markup_sub is not None:
        ...
        sub_ax_3.text(rob_x,rob_y,markup_sub,
                 ...
                 transform=ccrs.Robinson(), color='black',fontsize=12,bbox=dict(facecolor='white', alpha=1),fontweight='bold',zorder=25)
...
add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.52,-0.065,2,2],bounds=[-25, 106, -62.5, -79],label='Antarctica_Center',polygon=poly_ac,shades=True,markup='ANT (19)',markpos='right',markadj=0,markup_sub='c',sub_pos='mb')
...
add_inset(fig, [-179.99,179.99,-89.99,89.99], [-0.52, -0.2, 2, 2],bounds=[-82,-65,13,-57],label='Andes',markup='TRP (16) & SAN (17)',markadj=0,markup_sub='a',sub_pos='lm2')
add_inset(fig, [-179.99,179.99,-89.99,89.99], [-0.352, -0.365, 2, 2],bounds=[-100,-95,22,16],label='Mexico',markup_sub='b',sub_pos='rb')
add_inset(fig, [-179.99,179.99,-89.99,89.99], [-1.078, -0.215, 2, 2],bounds=[28,42,2,-5],label='Africa',markup_sub='c',sub_pos='rb')
add_inset(fig, [-179.99,179.99,-89.99,89.99], [-1.640, -0.275, 2, 2],bounds=[133,142,-2,-7],label='Indonesia',markup_sub='d',sub_pos='rb')

poly_arctic = np.array([(-105,84.5),(115,84.5),(110,68),(30,68),(18,57.5),(-70,57.5),(-100,75),(-105,84.5)])
add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.48,-1.005,2,2],bounds=[-100, 106, 57, 84],label='Arctic West',polygon=poly_arctic,markup='ARCTIC (03-09)',markadj=0)

add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.92,-0.17,2,2],bounds=[164,176,-47,-40],label='New Zealand',markup='NZL (18)',markpos='left',markadj=0)

poly_na = np.array([(-170,72),(-140,72),(-120,63),(-101,35),(-126,35),(-165,55),(-170,72)])
add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.1,-1.2,2,2],bounds=[-177,-105, 36, 70],label='North America',polygon=poly_na,markup='ALA (01) & WNA (02)',markadj=0)
...
add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.74,-1,2,2],bounds=[82,120,45.5,58.9],label='South Asia North',markup='ASN (10)',markup_sub='a',sub_pos='mb',markadj=0)
add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.517,-1.035,2,2],bounds=[53,70,62,69.8],label='North Asia West',markup_sub='b',sub_pos='lb',shades=False)

add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.685,-1.065,2,2],bounds=[65, 105, 46.5, 25],label='HMA',markup='HMA (13-15)',markadj=0)

add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.58,-0.982,2,2],bounds=[-4.9,19,38.2,50.5],label='Europe',markup='CEU (11)',markadj=0)

add_inset(fig,[-179.99,179.99,-89.99,89.99],[-0.66,-0.89,2,2],bounds=[38,54,29,45],label='Middle East',markup='CAU (12)',markadj=0)
...
axleg4.text(12500000, 200000, 'ASN (10)', fontsize=12,fontweight='bold')
...
axleg4.text(-16000000, 200000, 'TRP (16)', fontsize=12,fontweight='bold')
...
axleg4.text(-12200000, 200000, 'ANT (19)', fontsize=11,fontweight='bold')
