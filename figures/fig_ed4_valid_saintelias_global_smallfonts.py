...
ax2.imshow(hs_tmp[:, :], extent=ext, transform=ccrs.UTM(7), vmin=-50,vmax=50,cmap=cmap,interpolation='bilinear',zorder=2)

shape_feature = ShapelyFeature(Reader(fn_rgi_sub).geometries(), ccrs.PlateCarree(), edgecolor='grey', alpha=1,
                               facecolor='None',linewidth=0.25,zorder=3)
ax2.add_feature(shape_feature)
...
ax2.imshow(hs_tmp[:, :], extent=ext, transform=ccrs.UTM(7), vmin=0,vmax=2,cmap=cmap2,interpolation='nearest',zorder=4)

shape_feature = ShapelyFeature(Reader(fn_ic_diss).geometries(), ccrs.UTM(7), edgecolor='black', alpha=1,
                               facecolor='None',linewidth=0.35,linestyle='dashed',zorder=5)
ax2.add_feature(shape_feature)
...
ax2.imshow(hs_tmp[:, :], extent=ext, transform=ccrs.UTM(7), vmin=0,vmax=2,cmap=cmap2,interpolation='nearest',zorder=6)

shape_feature = ShapelyFeature(Reader(fn_ib_diss).geometries(), ccrs.UTM(7), edgecolor='black', alpha=1,
                               facecolor='None',linewidth=0.35,zorder=7)
ax2.add_feature(shape_feature)
...
ax2.add_patch(mpatches.Rectangle((605000,6539000),25000,5000,edgecolor='black',facecolor='black',transform=ccrs.UTM(7),zorder=10,linewidth=0.25))
ax2.add_patch(mpatches.Rectangle((630000,6539000),25000,5000,edgecolor='black',facecolor='white',transform=ccrs.UTM(7),zorder=10,linewidth=0.25))
...
ax2.text(298000,6656000,'                                                             \n                                                            \n                                                             ',bbox=dict(boxstyle='round', facecolor='white', alpha=0.7,linewidth=0.35),va='top',ha='left',zorder=11)
ax2.add_patch(mpatches.Rectangle((300000,6651000),25000,2000,edgecolor="black",facecolor=plt.cm.Purples(0.25),linewidth=0.35,zorder=12,linestyle='dashed'))
...
ax2.add_patch(mpatches.Rectangle((300000,6641000),25000,2000,edgecolor="black",facecolor=plt.cm.Purples(0.25),linewidth=0.35,zorder=12))
...
ax.text(-144,57,'Gulf\nof\nAlaska',ha='center',va='top',color='grey',transform=ccrs.PlateCarree(),fontsize=5,fontweight='bold')
...
ax.text(-130,62,'Saint-Elias\nMoutains',ha='left',va='center',color=plt.cm.Greys(0.9),transform=ccrs.PlateCarree(),fontsize=5,fontweight='bold',zorder=5)

ax.text(0.07, 0.05, 'a', transform=ax.transAxes,
        fontsize=8, fontweight='bold', va='bottom', ha='left',zorder=20)
...
ax.imshow(hs_land[:, :], extent=ext, transform=ccrs.Robinson(), cmap=cmap2, zorder=2,interpolation='nearest')
ax.imshow(hs_notland[:,:],extent=ext, transform=ccrs.Robinson(),cmap=cmap22, zorder=2,interpolation='nearest')
ds=None

shape_feature = ShapelyFeature(Reader(fn_shp).geometries(), ccrs.PlateCarree(), edgecolor='None', alpha=0.7,
                               facecolor='indigo',linewidth=0.35,zorder=3)
...
ax2.add_patch(mpatches.Polygon(polygon,edgecolor='grey',facecolor='dimgrey',zorder=11,alpha=0.25,linestyle='dotted',linewidth=0.5))

ax2.add_patch(mpatches.ConnectionPatch((501000-2000,6655000-2000),(501000+2000,6655000+2000),"data","data",color='black',linewidth=1.5,zorder=12))
ax2.add_patch(mpatches.ConnectionPatch((501000-2000,6655000+2000),(501000+2000,6655000-2000),"data","data",color='black',linewidth=1.5,zorder=12))
...
sub_ax3 = ax3.inset_axes([0.175,0.2,0.8,0.775])
...
p1=sub_ax3.plot(2000 + df_pred.t.values, df_pred.h_pred.values, lw=0.5, color='black',label='Time series')
p2=sub_ax3.errorbar(2000 + df_raw.t_raw.values, df_raw.h_raw.values, df_raw.err_raw.values, fmt='x', color=plt.cm.Oranges(0.95),elinewidth=0.5,markersize=2.5,label='Obs. (1$\sigma$ CI)')
p3=sub_ax3.scatter(2004.3,356.2,marker='d',color='black',s=15,zorder=30,label='ICESat')


sub_ax3.text(2004.3, 356.2-10,'z=0',bbox= dict(boxstyle='round', facecolor='white', alpha=0.8,linewidth=0.35),ha='center',va='top')
sub_ax3.text(2010, 358.2, 'z=1.5',bbox= dict(boxstyle='round', facecolor='white', alpha=0.8,linewidth=0.35),ha='left',va='center')
sub_ax3.tick_params(width=0.35, length=2.5)

# sub_ax3.vlines(2004.3,0,600,linestyles='dotted',colors='black')
sub_ax3.scatter(2008.8,358.2,marker='d',color='black',s=15,zorder=30)
# sub_ax3.vlines(2008.8,0,600,linestyles='dotted',colors='black')
p4=sub_ax3.scatter(2015.5,335.1,marker='o',color='black',s=15,zorder=30,label='IceBridge')
# sub_ax3.vlines(2015.5,0,600,colors='black')
sub_ax3.scatter(2019.8,310.3,marker='o',color='black',s=15,zorder=30)
...
ax0.text(0.02, 0.875, 'GLOBAL', transform=ax0.transAxes,
        fontsize=6, fontweight='bold', va='top', ha='left',zorder=20,rotation='vertical')
...
def add_inset(ax0,pos,df,bin_vec,xlab,cont=False,mu=True,mudt=False,ylab=True,add_mu_lab=None,month_xlab=False,draw_edges=True):
    ...
    for i in range(len(bin_vec)-1):
        ...
        if mu and ~np.isnan(df.med_dh.values[i]):
            ax.fill_between([i,i+1],[y_mu,y_mu],[y_mu+1,y_mu+1],facecolor=mpl.cm.RdYlBu(fun2(df.med_dh.values[i])),edgecolor='black',linewidth=0.25)
        elif mu:
            ax.fill_between([i, i + 1], [y_mu, y_mu], [y_mu+1, y_mu+1], facecolor='grey',edgecolor='black',linewidth=0.25)

        if mudt and ~np.isnan(df.dzsc_dt.values[i]):
            ax.fill_between([i,i+1],[y_mudt,y_mudt],[y_mudt+1,y_mudt+1],facecolor=mpl.cm.Oranges(fun3(np.abs(df.dzsc_dt.values[i]*df.nmad_dh.values[i]))),edgecolor='black',linewidth=0.25)
        elif mudt:
            ax.fill_between([i, i + 1], [y_mudt, y_mudt], [y_mudt+1, y_mudt+1], facecolor='grey',edgecolor='black',linewidth=0.25)

        if ~np.isnan(df.nmad_zsc.values[i]):
            ax.fill_between([i,i+1],[0,0],[1,1],facecolor=mpl.cm.Purples(fun(df.nmad_zsc.values[i])),edgecolor='black',linewidth=0.25)
        else:
            ax.fill_between([i, i + 1], [0, 0], [1, 1], facecolor='grey',edgecolor='black',linewidth=0.25)
        ...
        if draw_edges:
            for j in range(len(ticks)):
                ax.vlines(i, ticks[j] - 0.5, ticks[j] + 0.5, lw=0.25,color='black')
                ax.hlines([ticks[j] - 0.5, ticks[j] + 0.5], [i, i], [i + 1, i + 1], lw=0.25,color='black')
                if i == len(bin_vec) - 2:
                    ax.vlines(i + 1, ticks[j] - 0.5, ticks[j] + 0.5, lw=0.25,color='black')
...
def inset_hist(ax0,pos,df,bin_vec,xlab,val_mean=None,val_std=None,val_dmean=None,draw_edges=True,normal=True):
    ...
    if val_mean is not None:
        ax.vlines(fun(val_mean),0,4,lw=0.75,colors='black')
        ax.text(fun(val_mean)-1,3,'$\mu_{dh}$ =\n'+str(np.round(val_mean,2))+' m',va='center',ha='right',bbox=dict(boxstyle='round', facecolor=plt.cm.RdYlBu(0.5-0.47/10.), alpha=1,linewidth=0.35))
    if val_std is not None:
        ax.vlines(fun(val_std),0,4,lw=0.75,linestyles='dashed',colors='black')
        ax.vlines(fun(-val_std),0,4,lw=0.75,linestyles='dashed',colors='black')
        ax.text(fun(-val_std)-1,3,'$\sigma_{z}$ = '+str(np.round(val_std,2)),va='center',ha='right',bbox= dict(boxstyle='round', facecolor=plt.cm.Purples(0.47), alpha=1,linewidth=0.35))
    if val_dmean is not None:
        ax.text(fun(val_mean)+2.5,3,'$|d\mu_{dh}/dt|$ =\n'+str(np.round(val_dmean,3))+' m yr$^{-1}$',va='center',ha='left',bbox=dict(boxstyle='round', facecolor=plt.cm.Oranges(0.01/0.2), alpha=1,linewidth=0.35))
...
def inset_wls(ax0,pos,x,y,mu_y,beta2):
    ...
    ax.errorbar(x,y,mu_y,fmt='x',color=plt.cm.Greys(0.8),ms=2)

    yvec = (2000-x)*beta2[0]
    ax.plot(x,yvec,color='black')

    ax.text(2002.5,1.75,'$\\beta_{dh}$ = '+str(-np.round(beta2[0],3))+'\n$\pm$ '+str(np.round(beta2[1],3))+' m yr$^{-1}$',bbox=dict(boxstyle='round', facecolor=plt.cm.Oranges(0.001/0.2), alpha=1,linewidth=0.35))
...
plt.savefig('/home/atom/ongoing/work_worldwide/figures/final/ED_Figure_4.jpg',dpi=500)
