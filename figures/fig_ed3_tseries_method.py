...
color_list = ['tab:orange','tab:blue','tab:green','tab:cyan','tab:red','tab:purple','tab:brown','tab:pink','tab:gray','tab:olive']
...
for j in np.arange(1,nb_corr,2):
    ...
    ax.plot(bin_slope[:-2],nmad[np.arange(j,len(slope)-2*nb_corr,nb_corr)]**2,label='Empirical variance: q='+str(int(bin_corr[j]-5))+'-'+str(int(bin_corr[j]+5))+' %',color=color_list[u],linestyle=ls_list[i],marker='o')
    u+=1



model_var = np.sqrt(3**2 + ((20+(((100-np.array(100))/100)*20)) * np.tan(np.array(x_slope) * np.pi / 180))**2 + (((100-np.array(95))/100)*15)**2.5)
ax.plot(x_slope,model_var**2,label='Modelled: center of above categories',linestyle='dashed',color='black')
model_var = np.sqrt(3**2 + ((20+(((100-np.array(80))/100)*20)) * np.tan(np.array(x_slope) * np.pi / 180))**2 + (((100-np.array(75))/100)*15)**2.5)
ax.plot(x_slope,model_var**2,linestyle='dashed',color='black')
...
ax.legend(loc='upper center',title='Elevation measurement error',title_fontsize=12)
ax.grid()
...
for in_csv in list_in_csv:
    ...
    vmean=df['vmean']
    vstd=df['vstd']
    ax.plot(lags, vmean, lw=2, label='Empirical variogram',color='tab:blue')
...
ax.plot(sublag,white+local+period+linear, color='black', lw=2, linestyle='dashed',label = 'Modelled variogram: \nsum of individual models')

ax.plot(sublag,white, color='tab:cyan', label = 'Model: white noise',linestyle='dashed')
ax.plot(sublag,local, color='tab:red', label = 'Model: local',linestyle='dashed')
ax.plot(sublag,period, color='tab:gray', label = 'Model: periodic',linestyle='dashed')
ax.plot(sublag,linear, color='tab:orange', label = 'Model: linear',linestyle='dashed')
...
ax.set_ylabel('Variance of elevation differences (m$^{2}$)')
ax.legend(loc='upper center',title='Temporal covariance of glacier elevation',title_fontsize=12)
ax.set_xlim([0,12])
ax.grid()
ax.set_ylim([0,400])
# ax.set_xlim([0,10])
...
ax.plot(vect,np.ones(np.shape(vect))*(max_elev+cutoff_thr),lw=1.5,color='tab:orange')
ax.plot(vect,np.ones(np.shape(vect))*(min_elev-cutoff_thr),lw=1.5,color='tab:orange',label='Spatial filtering')
...
ax.plot(vect1, ref_vals - base_thresh - dt1*thresh[1],lw=1.5,color='tab:blue',label='Temporal filtering')
ax.plot(vect2, ref_vals + base_thresh - dt2*thresh[1],lw=1.5,color='tab:blue')
ax.plot(vect1, ref_vals + base_thresh - dt1*thresh[0],lw=1.5,color='tab:blue')
ax.plot(vect2, ref_vals - base_thresh - dt2*thresh[0],lw=1.5,color='tab:blue')
...
ax.scatter(ref_dem_date,ref_vals,100,color='tab:pink',label='Reference elevation (TanDEM-X)')
...
ax.legend(loc='upper right',title='Filtering with reference TanDEM-X',title_fontsize=12)
ax.grid()
...
while (n_out > 0 or final_fit==0) and num_finite >= 2:
    ...
    if niter == 4:
        ax.plot(y0+detr_t_pred + mu_x, y_pred + mu_y, lw=2, label='Successive Gaussian Process regression fits', color=plt.cm.Greys(0.8),linewidth=0.8)
        ...
        ax.fill_between(y0+detr_t_pred +mu_x, mu_y + y_pred.squeeze() + max_z_score[min(niter, len(max_z_score) - 1)] * sigma.squeeze(),
                        mu_y + y_pred.squeeze() - max_z_score[min(niter, len(max_z_score) - 1)] * sigma.squeeze(), facecolor=plt.cm.Blues(0.2+niter/10), alpha=0.8,label='Successive GP confidence intervals\nof 20, 12, 9, 6 and 4 sigmas')
    else:
        ax.plot(y0+detr_t_pred + mu_x, y_pred + mu_y, lw=2, color=plt.cm.Greys(0.8),linewidth=0.8)
...
ax.set_xlabel('Year')
ax.set_ylabel('Elevation (m)')
ax.legend(loc='upper right',title='Filtering with iterative GP regression',title_fontsize=12)
ax.grid()
ax.set_ylim([140,700])
...
ax.text(0.025, 0.95, 'e', transform=ax.transAxes,
        fontsize=16, fontweight='bold', va='top', ha='left')
ax.plot(y0+detr_t_pred + mu_x, y_pred + mu_y, lw=2, label='GP regression time series', color='blue')
...
ax.fill_between(y0+detr_t_pred +mu_x, mu_y + y_pred.squeeze() + sigma.squeeze(),
                        mu_y + y_pred.squeeze() - sigma.squeeze(), facecolor='blue', alpha=0.5, label='1$\sigma$ confidence interval')
ax.set_xlabel('Year')
ax.set_ylabel('Elevation (m)')
ax.legend(loc='upper right',title='Final time series with GP regression',title_fontsize=12)
# ax.set_ylim([120,580])
ax.set_xticks(np.arange(2000, 2024, 4))
ax.grid()

plt.savefig('/home/atom/ongoing/work_worldwide/figures/revised/ED_Figure_3.png',dpi=300)