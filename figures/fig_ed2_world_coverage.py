...
cmap2 = mpl.colors.LinearSegmentedColormap.from_list('cmap1', ['white', 'tab:blue'], 100)
...
sub_ax.imshow(np.flip(hs_tmp_l[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmaphs2, zorder=2)
sub_ax.imshow(np.flip(hs_tmp_nl[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmaphs22, zorder=2)

sub_ax.outline_patch.set_edgecolor('lightgrey')

sub_ax.imshow(np.flip(hs_tmp[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmap2, zorder=2,vmin=0,vmax=100, resample=True)
...
sub_ax_2.imshow(np.flip(hs_tmp_l[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmaphs2, zorder=2)
sub_ax_2.imshow(np.flip(hs_tmp_nl[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmaphs22, zorder=2)

sub_ax_2.outline_patch.set_edgecolor('lightgrey')

sub_ax_2.imshow(np.flip(hs_tmp[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmap, zorder=2,vmin=0,vmax=200, resample=True)
...
cmap3 = mpl.colors.LinearSegmentedColormap.from_list('cmap1', ['white', 'tab:green'], 50)
...
sub_ax_3.imshow(np.flip(hs_tmp_l[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmaphs2, zorder=2)
sub_ax_3.imshow(np.flip(hs_tmp_nl[:, :], axis=0), extent=ext, transform=ccrs.Robinson(), cmap=cmaphs22, zorder=2)

sub_ax_3.outline_patch.set_edgecolor('lightgrey')
sub_ax_3.imshow(np.flip(hs_tmp2[:, :], axis=0), extent=ext2, transform=ccrs.Robinson(), cmap=cmap3, zorder=2,vmin=0,vmax=50, resample=True)
...
cb.set_label('ArcticDEM strip count',fontsize=12)
cb2.ax.tick_params(labelsize=12)
cb2.set_label('REMA strip count',fontsize=12)
...
cb3.set_label('ASTER strip count',fontsize=12)
...
ax2.hist([aster_dec,arc_dec,rema_dec],120,color=['tab:orange','tab:blue','tab:green'],label=['ASTER: 154,565','ArcticDEM: 67,986','REMA: 9,369'],edgecolor='black',lw=0.5,stacked=True,orientation='horizontal')
ax2.set_xlabel('Bi-mensual strip count',fontsize=12)
# ax2.set_ylabel('Time')
...
# plt.savefig('/home/atom/ongoing/work_worldwide/figures/revised/ED_maybe.png',dpi=400)